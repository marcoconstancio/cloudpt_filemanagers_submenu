...
class CloudPtExtension(caja.MenuProvider):
    home_dir = expanduser("~")
    extension_dir = os.path.join(home_dir,".local","share","caja-python","extensions")
    ...
    def get_file_items(self, window, files):
        ...
        if files:
            ...
            if self.config_data['cloudpt_dir'] == parent_dir:
                ...
                top_menuitem = caja.MenuItem(name='CajaPython::cloudpt_folder',label='CloudPT',tip='',icon=os.path.join(self.extension_dir,'cloudpt.png'))
                submenu = caja.Menu()
                top_menuitem.set_submenu(submenu)

                for menu_item_info in menu_layout:
                    sub_menuitem = caja.MenuItem(name='CajaPython::'+menu_item_info[1], label=menu_item_info[1], tip='', icon=menu_item_info[2])
    ...
    def get_background_items(self, window, file):
        ...
        if self.config_data['cloudpt_dir'] == current_dir[:self.config_data['cloudpt_dir_len']]:
            ...
            menuitem = caja.MenuItem(name='CajaPython::cloudpt_files',label='CloudPT',tip='',icon=os.path.join(self.extension_dir,'cloudpt.png'))
